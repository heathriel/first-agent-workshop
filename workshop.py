...
def main():
    ...
    if args.command == 'preflight':
        ...
        print('CORE PREFLIGHT')
        print('Python 3.9+:', 'PASS (' + sys.version.split()[0] + ')' if python_ok else 'NOT READY (' + sys.version.split()[0] + ')')
        print('Git:', 'PASS (' + git_path + ')' if git_path else 'OPTIONAL / NOT FOUND (Download ZIP or use the manual lane)')
        print('Guided Claude lane:', 'CLI FOUND (' + claude_path + '); sign-in still needs a live hello-file test' if claude_path else 'NOT READY (pair, use another assistant, or use the manual lane)')
        ...
        print('Fixture files:', 'PASS' if not missing else 'NOT READY: ' + ', '.join(missing))
        if python_ok and not missing:
            print('CORE FILES: PASS')
        else:
            print('CORE FILES: NOT READY; pair or use cheatsheets/universal.md')
        print('Next: ask your agent to create output/hello.md containing hi, then read it back.')
        print('This script does not check account sign-in or service access.')
        return int(bool(missing) or not python_ok)
